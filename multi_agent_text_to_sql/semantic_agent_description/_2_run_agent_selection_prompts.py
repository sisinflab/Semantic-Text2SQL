...
PROMPTS_PATH = Path("_1_generated_semantic_selection_prompts_by_id.json")
DEFAULT_MAX_TOKENS = int(os.getenv("VLLM_MAX_TOKENS", "8192"))
DEFAULT_TEMPERATURE = float(os.getenv("VLLM_TEMPERATURE", "0.0"))

# ===== GESTIONE BATCH (Aggiunta) =====
# Es: esporta export START_IDX=0 ed export END_IDX=500 prima di lanciare lo script
START_IDX = int(os.getenv("START_IDX", "0"))
END_IDX = os.getenv("END_IDX")
END_IDX_VAL = int(END_IDX) if END_IDX else None
...
def load_prompts_payload(path: Path) -> list[dict]:
    ...
    # ===== APPLICAZIONE SLICE BATCH =====
    logger.info(f"Filtro batch applicato: elementi da {START_IDX} a {END_IDX_VAL if END_IDX_VAL is not None else 'fine'}")
    return sorted_records[START_IDX:END_IDX_VAL]
...
def main() -> int:
    ...
    if not prompts_data:
        print("ERRORE: Nessun prompt valido trovato nel file input per questo batch.")
        return 1

    model_name = os.path.basename(model_path.rstrip("/\\"))
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"_2_vllm_semantic_agent_selection_outputs_{model_name}_batch_{START_IDX}_{END_IDX_VAL}_{timestamp}.json"
    ...
    logger.info(f"Prompt totali nel batch: {len(prompts_data)}")
    ...
    for idx, (out, item) in enumerate(zip(outputs, prompts_data)):
        ...
        # Recupera i concetti direttamente dalla voce della entry corrente
        available_concepts = item.get("available_semantic_concepts", [])
    ...
    with open(DEBUG_FILE, "w", encoding="utf-8") as f:
        json.dump({
            "analysis": {
                ...
                "avg_prompt_length": sum(d["prompt_length_chars"] for d in debug_info) // len(debug_info),
                "max_prompt_length": max(d["prompt_length_chars"] for d in debug_info),
                "model_config": {
                    "max_tokens": DEFAULT_MAX_TOKENS,
                    "temperature": DEFAULT_TEMPERATURE,
                }
