...
def format_user_content_with_evidence_and_types(schema_linking, question,evidence,types_info):
    return f"""Task Overview:
    You are a data science expert. Below, you are provided with examples, a database schema and a natural language question. Your task is to understand the schema and generate a valid SQL query to answer the question.
    ...
    this informatios is about the data type of colums in the database, it can be useful to understand how to write the query.
...
def generate_sql_batch(llm, tokenizer, sampling_params, questions, db_names, schema_linking_list,ids,evidence_list, types_info_list):
    ...
    for question, db_name, schema_linking,id,evidence, types_info in zip(questions, db_names, schema_linking_list,ids,evidence_list, types_info_list):
        ...
        user_message= format_user_content_with_evidence_and_types(schema_linking, question,evidence, types_info)
...
if __name__ == '__main__':
    ...
    dataset_path = '_4_source_dataset_with_columns_by_type.json'
    ...
    types_info_list = [ex['columns_by_type'] for ex in test_data]

    # Generate all predictions in batch
    print("Generating SQL queries in batch...")

    start = datetime.datetime.now()
    pred_sqls = generate_sql_batch(llm, tokenizer, sampling_params, questions, db_names, schema_linking, ids, evidence_list, types_info_list)
    ...
    print(f"Time taken for SQL generation: {(end - start).total_seconds():.2f} seconds")
    print(f"Average time per example: {(end - start).total_seconds() / total:.2f} seconds")


    OUTPUT_FILENAME= '_5_vllm_pred_sqls.json'
